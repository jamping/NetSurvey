...
void CUserManageDlg::OnAddUser() 
{
	...
	if(MessageBox("创建用户: "+m_strUserName+" 确定吗？","请确认",MB_YESNO |MB_ICONQUESTION)==IDYES)
	{
		...
		strSql.Format("insert into netsurvey_user (user_name,user_passwd,user_level,user_desc,create_time) \
		...
		if(theApp.GetDB()->Execute(strSql))
		{
			...
			m_listUsers.AddItem(m_strUserName,des.DesCrypteString(_T("auqfvfhpgu"),m_strUserPasswd,1),
				theApp.GetUserDescName((CNetSurveyApp::UserLevelType)m_nUserLevel),
...
void CUserManageDlg::OnClearUsers() 
{
	...
	if(MessageBox("删除所有用户，确定吗？","请确认",MB_YESNO |MB_ICONQUESTION)==IDYES)
	{
        ...
        CString strSql=_T("delete from netsurvey_user");
		if(theApp.GetDB()->Execute(strSql))
		{
	...
	{		
		...
		for (i=0;i < uSelectedCount;i++)
		{
			...
			if(nItem != -1)
			{
				...
				strSql.Format("delete from netsurvey_user where user_name='%s'",strUserName);
...
void CUserManageDlg::OnModifyUser() 
{
	...
	COleDateTime time=COleDateTime::GetCurrentTime();
	CDes des;
	strSql.Format("update netsurvey_user set user_name='%s',user_passwd='%s',user_level=%d,user_desc='%s', \
    ...
    if(theApp.GetDB()->Execute(strSql))
	{
		...
		m_listUsers.SetItemText(m_nCurSelectedItem,2,theApp.GetUserDescName((CNetSurveyApp::UserLevelType)m_nUserLevel));
...
BOOL CUserManageDlg::OnInitDialog() 
{
	...
	strSql=_T("select * from netsurvey_user order by user_level desc");
	...
	if(rs.Open(strSql,CADORecordset::openQuery))
	{
		...
		{	
			...
			m_listUsers.SetItemText(nItem,2,theApp.GetUserDescName((CNetSurveyApp::UserLevelType)nLevel));
