...
CLogsDlg::CLogsDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CLogsDlg::IDD, pParent),m_rs(theApp.GetDB())
{
...
void CLogsDlg::OnClearLogs() 
{
	...
	if(MessageBox("清空所有日志吗？","请确认",MB_YESNO |MB_ICONQUESTION)==IDYES)
	{
        ...
        CString strSql=_T("delete from netsurvey_log");
...
BOOL CLogsDlg::OnInitDialog() 
{
	...
	strSql=_T("select count(*) as total from netsurvey_log ");

	if(m_rs.Open(strSql,CADORecordset::openQuery))
	{
		long  lTotal=0;
		m_rs.GetFieldValue(_T("total"),lTotal);
...
void CLogsDlg::ShowContent()
{
	...
	if(m_nCurPage==1)
	{
		strSql.Format("select top %d * from netsurvey_log order by log_time desc",NUM_PER_PAGE);
	}
	else
	{
		strSql.Format("select top %d * from netsurvey_log where log_time < \
			 (select min (log_time) from (select top %d log_time from netsurvey_log order by log_time desc ) as T ) \
			 order by log_time desc",NUM_PER_PAGE,(m_nCurPage-1)*NUM_PER_PAGE);
	}
